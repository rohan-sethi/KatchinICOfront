...
var userFun =module.exports = {
    ...
    authenticateCredential: (req, res, next) => {
        let fetchedUser;
        let token;
        User.findOne({ email: req.body.email,status:'active'}).then(userRes => {
    ...
    authenticateUser: (req, res, next) =>{
        ...
        let verified = false;
        console.log("caaled");
        User.findOne({ email: req.body.email ,status:'active'}).then(user => {
            console.log(user);
            if (!user) {
                res.status(401).json({
                    message: "Either email or password wrong",
            ...
            .then(wallet => {
                ...
                if (wallet != null){
                    ...
                    bcrypt.hash(req.body.email, 10)
                        .then((hashedPassword) => {
                            secretToken = speakeasy.generateSecret({ length: 20 });
                            console.log(wallet);
                            user = new User({
                                email: req.body.email,
                                activateUrl: secretToken.base32+Date.now(),// url render. creating issue 
                              ...
                              //  username: req.body.username,
                                ...
                                salt: secretToken.base32,
                                oauthUrl: secretToken.otpauth_url,
                                keys: [],
                                status: 'sendPassword'
                            });
                            ...
                            task
                              ...
                              .then(result => {
								 ...
								 // set opt 
								const otp = new UserOtp({ otp: user.activateUrl, sendAt: Date.now(), otpType: "password", userId: result[0]._id });
								otp.save()
								.then(resultRes => {								
									...
									text: 'To finish activating your account, you must click this link: http://172.104.173.247:3000/set_password/'+user.activateUrl
									};
		...
		.then(user => {
			if (!user) {
				return res.status(200).json(
				 ...
				 secretToken = speakeasy.generateSecret({ length: 20 });
				  userToken =secretToken.base32+Date.now(); // url render. creating issue 				  
				  
					User.update({id:user._id},{activateUrl: userToken})
						.then(result => {
						 // set opt 
							const otp = new UserOtp({ otp: user.activateUrl, sendAt: Date.now(), otpType: "password", userId: user._id });
							otp.save()
							.then(resultRes => {								
								...
								text: 'To set your password , you must click this link: http://172.104.173.247:3000/set_password/'+user.activateUrl
								};
    ...
    enableTwoFa: (req, res, next) =>{
		...
		User.findOne({email: req.body.email})
			.then(user => {
				...
				User.update({id:user._id},{twoFactor: true})
					.then(result => {
						console.log(result);
	...
	activateUser: (req, res, next) => {		
		...
		UserOtp.findOne({ otp: req.body.token, otpType: "password" }).sort({ field: 'asc', _id: -1 })
                    .then(userOtpRes => {
						console.log(userOtpRes)
                        if (((Date.now() - Number(userOtpRes.sendAt)) / (1000 * 60)) <= 15000 && userOtpRes.otp === req.body.token) {
                          verified = true;
                        } else {
                          verified = false;
                        }
                        ...
                        if (verified == true) {
							...
							{	
							data:userOtpRes,
							message: "Your token is valid Please Set password.",
							success: true
							});
	 ...
	 requestDisbaleTwoFa: (req, res, next) => {
        ...
        User.findOne({ email: req.body.email }).then(user => {
            ...
            otp.save()
                .then(result => {
                    console.log(result)
                    res.status(200).json({ key: requestKey });
                });
        })
        .catch(error => {
            res.status(500).json({
                ...
                .then(otp => {
                    if(((Date.now() - Number(otp.sendAt)) / (1000 * 60 )) > 15){
                        ...
                        if(requestValue === req.body.value){
                            User.update({ _id: fetchedUser._id }, { twoFactor: false})
                                .then(result => {
                                    if(result.nModified > 0){
                                        res.status(200).json({
                                        ...
                                        res.status(500).json({
                                            message: "Error occured while disabling the 2fa"
                                        })
                                    }
                                })
                        }else{
